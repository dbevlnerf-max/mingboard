...
const PRIMARY_GUILD_ID =
  process.env.MINGBOT_PRIMARY_GUILD_ID?.trim() ||
  "1519294285185417306";

function unauthorized() {
  return NextResponse.json(
    { success: false, message: "밍봇 API 인증에 실패했습니다." },
    { status: 401 }
  );
}

function forbiddenGuild(guildId: string) {
  return NextResponse.json(
    {
      success: false,
      message: "운영 Guild가 아닙니다.",
      guildId,
      primaryGuildId: PRIMARY_GUILD_ID,
    },
    { status: 403 }
  );
}
...
function isPrimaryGuild(guildId: string) {
  return Boolean(guildId && guildId === PRIMARY_GUILD_ID);
}
...
async function getCurrentState(bossId: string) {
  const { data, error } = await supabaseAdmin
    .from("boss_timer_states")
    .select("boss_id,last_spawn_at,next_spawn_at,source,updated_at")
    .eq("boss_id", bossId)
    .maybeSingle();
...
async function saveMingbotStateIfNotOlder(
  bossId: string,
  nextSpawnAt: string,
  actorDiscordId: string | null,
  lastSpawnAt: string | null = null,
  externalEventId: string | null = null
) {
  const current = await getCurrentState(bossId);
  const incomingMs = new Date(nextSpawnAt).getTime();
  const currentMs = current?.next_spawn_at
    ? new Date(current.next_spawn_at).getTime()
    : null;
  ...
  if (
    ...
    return {
      ignoredAsStale: true,
      currentNextSpawnAt: current?.next_spawn_at || null,
    };
  }
    ...
    .upsert(
      {
        ...
        updated_by_discord_id: actorDiscordId,
      },
      { onConflict: "boss_id" }
    );

  if (error) throw error;
  return { ignoredAsStale: false, currentNextSpawnAt: nextSpawnAt };
}
...
async function upsertBoss(body: Record<string, unknown>) {
  ...
  if (!isPrimaryGuild(guildId)) {
    return { ok: false, status: 403, message: "운영 Guild가 아닙니다." };
  }
  ...
  if (level === undefined) {
    return { ok: false, status: 400, message: "보스 레벨이 올바르지 않습니다." };
  }
  ...
  if (nextSpawnAt) {
    stateResult = await saveMingbotStateIfNotOlder(
      boss.id,
      nextSpawnAt,
      cleanText(body.actorDiscordId) || null
    );
  }
...
async function disableBoss(body: Record<string, unknown>) {
  ...
  if (!boss) {
    return {
      ok: true,
      status: 200,
      data: { success: true, alreadyDisabled: true, guildId, name },
    };
  }
  ...
  return {
    ok: true,
    status: 200,
    data: { success: true, alreadyDisabled: false, guildId, name },
  };
}
...
async function updateBossState(body: Record<string, unknown>) {
  ...
  if (!boss) {
    return { ok: false, status: 500, message: "보스 자동 등록 후 정보를 찾지 못했습니다." };
  }

  const stateResult = await saveMingbotStateIfNotOlder(
    boss.id,
    nextSpawnAt,
    cleanText(body.actorDiscordId) || null,
    occurredAt,
  ...
  if (stateResult.ignoredAsStale) {
    return {
      ...
      data: {
        ...
        guildId,
        name,
        currentNextSpawnAt: stateResult.currentNextSpawnAt,
      },
    };
  }
  ...
  const { data: rpcResult, error: rpcError } = await supabaseAdmin.rpc(
    "record_boss_timer_event",
    {
      ...
      p_next_spawn_at: nextSpawnAt,
      p_source: "mingbot",
      p_external_event_id: externalEventId,
      p_actor_discord_id: cleanText(body.actorDiscordId) || null,
      p_event_type: cleanText(body.eventType) || "cut",
    }
  );
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!guildId) {
      return NextResponse.json(
        { success: false, message: "guildId가 없습니다." },
        { status: 400 }
      );
    }
    if (!isPrimaryGuild(guildId)) return forbiddenGuild(guildId);
    ...
    return NextResponse.json(
      { success: false, message: "밍봇 보스설정 조회 중 오류가 발생했습니다." },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const guildId = getGuildId(body);
    if (guildId && !isPrimaryGuild(guildId)) return forbiddenGuild(guildId);
    ...
    if (action === "upsert") result = await upsertBoss(body);
      ...
      return NextResponse.json(
        { success: false, message: "지원하지 않는 보스타임 작업입니다." },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { success: false, message: "밍봇 보스타임 동기화 중 오류가 발생했습니다." },
      { status: 500 }
    );
  }
}
